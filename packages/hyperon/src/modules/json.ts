...
/**
 * The JSON module, modeled on hyperon-experimental's `hyperon.stdlib` JSON operations. Register it
 * on a {@link MeTTa} runner to make these grounded operations callable from MeTTa source:
 *
 ...
 * - `json-decode` `(-> String Atom)` — JSON text to MeTTa (array→expression, object→dict-space,
 *   string→String, number→Number, bool→Bool, null→`null`).
...
function atomToJson(atom: Atom): unknown {
  ...
  if (atom instanceof GroundedAtom) {
    ...
    if (content !== null && t !== "string" && t !== "number" && t !== "boolean")
      throw new Error(`json-encode: grounded value of type '${t}' is not JSON-encodable: ${atom.toString()}`);
    return content;
  }
...
// A counter for fresh internal query variables, so get-value never collides with a key's own vars.
let freshVar = 0;
...
export function registerJsonModule(m: MeTTa): void {
  ...
  m.registerOperation("get-keys", (args) => {
    ...
    if (space === undefined)
      throw new Error(`get-keys: expected a dict-space, got ${args[0]?.toString() ?? "no argument"}`);
    const keys: Atom[] = [];
    for (const a of space.getAtoms())
      if (a instanceof ExpressionAtom) {
  ...
  m.registerOperation("get-value", (args) => {
    ...
    const key = args[1];
    if (space === undefined)
      throw new Error(`get-value: expected a dict-space, got ${args[0]?.toString() ?? "no argument"}`);
  ...
  m.registerOperation("json-decode", (args) => {
    ...
    if (!(a instanceof GroundedAtom))
      throw new Error(`json-decode: expected a grounded string, got ${a?.toString() ?? "no argument"}`);
