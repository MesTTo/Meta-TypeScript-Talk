...
/**
 * The atom API: an idiomatic TypeScript class surface over the functional `@metta-ts/core` term
 * model, modeled on Hyperon's `hyperon.atoms`. This is a TypeScript-native library (no Python, no
 * Rust, no FFI), so it wraps the core's immutable atoms in classes rather than bridging a foreign
 * runtime. Python method names are kept as aliases (`get_name`, `get_metatype`, ...) next to the
 * idiomatic ones (`name`, `metatype`, ...) so ported code reads naturally either way.
 */
...
/**
 * Base class for every atom. Wraps one immutable core atom (`catom`) and exposes the operations
 * common to all kinds: metatype, structural equality, rendering, depth-first iteration, and matching.
 */
export abstract class Atom {
  ...
  /** A human-readable MeTTa rendering. */
  toString(): string {
...
/** Guard a wrapper subclass against a core atom of the wrong kind (a constructor footgun). */
function assertKind(catom: core.Atom, kind: core.Atom["kind"], cls: string): void {
  if (catom.kind !== kind)
    throw new Error(`${cls} expects a '${kind}' atom, got '${catom.kind}'`);
}
...
export class OperationObject extends GroundedObject {
  constructor(
    ...
    // Accepted for source-compatibility with Python Hyperon's `OperationAtom(name, fn, unwrap)`, but
    // ignored: a TypeScript operation always receives the argument atoms (`Atom[]`), never unwrapped
    // JS values, so there is nothing to toggle.
...
// A registry so a grounded atom built from a JS object can recover that object later. The core's
// grounded `ext` value carries only an id; the object lives here. The entry must outlive the wrapper
// (a core atom may sit in a Space and be re-wrapped after the original wrapper is gone), so the
// registry is process-global and grows with each `G()`/`OperationAtom()`/object-valued `ValueAtom()`.
// Long-running hosts that create many grounded objects can reclaim memory with
...
/** A grounded atom: sub-symbolic content (a value, a space, or an operation) wrapped as an atom. */
export class GroundedAtom extends Atom {
  ...
  /** The plain JS value behind this grounded atom (its object's `content`), typed as `T`. A typed
   *  shortcut for `object().content` so callers avoid the `as` cast at every use site. */
  jsValue<T = unknown>(): T {
 ...
 *  becomes an *executable* grounded atom: when it heads an expression `(<atom> arg...)` the interpreter
 *  runs its operation (this is what makes `OperationAtom` callable in-language, like Python's py-atom). */
export function G(obj: GroundedObject, type?: Atom): GroundedAtom {
  ...
  const exec =
    obj instanceof OperationObject
      ...
      : undefined;
  // A MatchableObject defines custom unification: wire its `match_` into the core's grounded match.
  const match =
...
/** A human-friendly type name for an atom, for use in error messages (e.g. `Number`, `String`,
