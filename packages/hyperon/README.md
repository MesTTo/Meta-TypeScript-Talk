...
and `hyperon.base`. Where the Python package wraps a Rust core over FFI, this one is TypeScript over
TypeScript: it wraps the immutable terms of [`@metta-ts/core`](../core) in classes. The result runs
anywhere TypeScript runs, with no native addon and no WASM.
...
The `MeTTa` runner evaluates programs and keeps its knowledge base across `run` calls, REPL-style.
...
You can extend the runner. `registerOperation(name, fn)` adds a grounded operation callable from
...
The runner's `space()` is **live**, not a snapshot: an atom added through it reaches the evaluator
...
custom-matching value. `ValueAtom` converts primitives for you (`number` → Number, `string` →
String, `boolean` → Bool); `G(obj, type?)` wraps any `GroundedObject`, and `GroundedAtom.object()`
recovers it.
...
engine *is* TypeScript, so there's no bridge: a grounded atom can hold a JS function and the interpreter
...
function); `js-dot` reads a property/method (methods come back bound to their object); `js-list` and
`js-dict` build a JS array / object from MeTTa atoms.

This rests on **executable grounded atoms**: an `OperationAtom` that heads an expression is run by the
interpreter, so a JS function wrapped as an atom is callable in-language. That's also what makes
`registerAtom` + an `OperationAtom` work like Python's `bind! abs (py-atom ...)`:
...
A grounded operation that throws surfaces as a MeTTa `(Error ...)` atom (a hard, visible error the
program can still inspect) rather than crashing the run. `evaluateAtom(atom)` evaluates a single
constructed atom (the atom-level counterpart of `run`).
