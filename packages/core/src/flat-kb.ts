...
import { type Atom, type Ground, sym, variable, expr, gnd, groundEq } from "./atom";
...
export function encodeInto(a: Atom, out: number[], it: Interner, varMap: Map<string, number>): void {
  switch (a.kind) {
    case "sym":
...
function rangeEq(a: Int32Array | number[], as: number, b: Int32Array | number[], bs: number): boolean {
 ...
 *  Bruijn order, or `null` if a pattern symbol/ground is absent from the interner (so it can never match
 *  any stored fact — a fast fail). */
...
export class FlatKB {
  ...
  // functor id (or a sentinel for non-expr/var-headed) -> list of fact start offsets.
  ...
  private candidates(pattern: Atom): number[] | "all" {
    const head =
      pattern.kind === "expr" && pattern.items.length > 0 ? pattern.items[0] : pattern;
  ...
  match(pattern: Atom): Array<Map<string, Atom>> {
    ...
    const offsets = cand === "all" ? this.allOffsets() : cand;
    ...
    for (const off of offsets) {
      ...
      for (const [idx, [s]] of binds) m.set(enc.varNames[idx]!, decodeAt(this.tokens, s, this.interner)[0]);
      out.push(m);
    }
    return out;
  }

  private allOffsets(): number[] {
    return this.offsets;
  }
...
export const _internals = { tok, tagOf, payloadOf, groundEq, subtermLen };
