...
// Compare two rankings as multisets keyed by (printed pattern, count, len, gain) — robust to tie order.
  ...
  for (const { name, facts } of corpora)
    for (const refCost of [1, 4, 8])
      it(`${name} @ refCost=${refCost} matches the oracle`, () => {
        ...
        expect(asSet(williamTopK(kb, 100, refCost))).toEqual(asSet(oracleTopK(facts, 100, refCost)));
      });
});
...
describe("william top-k — economics and ranking", () => {
  it("surfaces the heaviest repeated subterm first", () => {
    const kb = new FlatKB();
    for (const tgt of ["b", "c", "d"]) kb.add(A(sym("link"), A(sym("node"), sym("a")), A(sym("node"), sym(tgt))));
