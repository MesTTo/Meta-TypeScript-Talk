...
describe("flat-kb encode/decode round-trip", () => {
  ...
  it("decodes distinct fresh variables to distinct de Bruijn names (not all $0)", () => {
    ...
    expect(format(decodeAtom(encodeAtom(A(sym("f"), variable("x"), variable("y")), it), it))).toEqual(
      "(f $0 $1)",
    );
    // A repeated variable shares one name (alpha-equivalent to the original).
    expect(format(decodeAtom(encodeAtom(A(sym("g"), variable("x"), variable("x")), it), it))).toEqual(
      "(g $0 $0)",
    );
  });
});
...
function flatMatch(facts: Atom[], pattern: Atom): string[] {
  ...
  return kb
    .match(pattern)
    .map((m) => atomVars(pattern).map((v) => `${v}=${format(m.get(v) ?? variable(v))}`).join(","))
    .sort();
}
...
describe("flat-kb scaling", () => {
  ...
  it("matches a single-functor query correctly (linear scan over the functor bucket)", () => {
    ...
    expect(kb.match(A(sym("edge"), gint(5_000), variable("y"))).map((m) => format(m.get("y")!))).toEqual([
      "5001",
    ]);
  });
});
