...
// @metta-ts/browser — the web-native entry point. The core interpreter is already pure (no Node
// dependencies), so the browser build re-exports it and adds an in-memory VFS for `import!`.
import {
  type Atom,
  parseAll,
  standardTokenizer,
...
/** Run a MeTTa program in the browser, with optional in-memory modules for `import!`. */
export function run(
  src: string,
