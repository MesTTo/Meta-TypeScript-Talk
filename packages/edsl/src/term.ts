...
// The term layer of the eDSL: typed builders that produce ordinary MeTTa atoms. Everything here returns
// a `@metta-ts/hyperon` atom, so the result runs unchanged through the existing interpreter.
//
// Two conventions make this ergonomic and unambiguous:
...
//     term is expected is auto-grounded into a grounded atom. That is what lets you drop a TypeScript
//     object straight into a rule or query.
import { Atom, S as hS, V, E, ValueAtom, type SymbolAtom, type VariableAtom, type ExpressionAtom } from "@metta-ts/hyperon";

/** A typed variable. The phantom `T` is the JS type its binding unwraps to in a query result; it is a
 *  promise you make to the type checker (the runtime value is whatever actually unifies), in the spirit
 *  of a typed query builder where you bring your own types. */
 ...
 *  This is the single primitive behind "pass a TypeScript object directly into a rule or query". */
