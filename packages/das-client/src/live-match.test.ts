...
import { sym, variable, expr, gstr, format, type Atom } from "@metta-ts/core";
...
describe("DasLiveSpace.matchAsync", () => {
  run("returns the eight animal concepts as atoms", async () => {
    ...
    const expected = ["chimp", "earthworm", "ent", "human", "monkey", "rhino", "snake", "triceratops"]
      .map((n) => `"${n}"`)
      .sort();
  ...
  run("handles two concurrent queries without a port or node-id collision", async () => {
    ...
    const q = (pred: string): Promise<string[]> =>
      matchAsync(
        space,
        expr([sym("EVALUATION"), expr([sym("PREDICATE"), gstr(pred)]), expr([sym("CONCEPT"), C])]),
        C,
