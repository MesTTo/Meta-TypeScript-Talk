...
describe("live DAS pattern_matching_query", () => {
  run("issues (EVALUATION (PREDICATE is_animal) (CONCEPT $C)) and decodes the answers", async () => {
    // String literals are stored quoted, so the node names carry their surrounding quotes.
    const pattern = expr(
      node("EVALUATION"),
